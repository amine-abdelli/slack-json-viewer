...
function DialogContent({
  ...
  return (
    <DialogPortal>
      <DialogOverlay />
      {/*
        One z-index above the overlay: Radix portals the overlay and the content
        separately, so when a dialog that stays mounted reopens, the overlay can
        land after the content in <body> and, at an equal z-index, cover it.
      */}
      <DialogPrimitive.Content
        data-slot="dialog-content"
        className={cn(
          "bg-surface data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-[50%] left-[50%] z-[51] grid w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] gap-4 rounded-xl border p-6 shadow-3 duration-200 sm:max-w-lg",
          className
        )}
        {...props}
      >
