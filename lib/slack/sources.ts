/**
 * Where an import reads from: the server bridge (token and cookie, or QR), or
...
export interface SlackSource {
  ...
  dump(channel: string, onLog: Log, signal: AbortSignal): Promise<unknown>;
...
export function bridgeSource(workspace: string): SlackSource {
  return {
    ...
    dump: (channel, onLog, signal) =>
      runJob<unknown>({ action: "dump", workspace, channel }, onLog, signal),
...
export function extensionSource(team: ExtensionTeam, i18n: I18n): SlackSource {
  ...
  return {
    ...
    dump: (channel, onLog, signal) => core.dumpConversation(ctx, channel, onLog, signal),
